...
tokens = [
    "27ecb87a-1c4c-4b24-940d-2ad04b4dc5a7"
]
...
def clean_expired_image_tokens():
    while True:
        ...
        # 清理120秒（2分钟）前的令牌
        expired_tokens = [t for t, (created_time, _) in one_time_image_tokens.items() if current_time - created_time > 120]
...
async def api_get_official(station_name: str,timestamp:int = None,element: str = "all",token:str = Depends(verify)):
    #未指定时间戳时，返回最新数据
...
async def api_get_image(mode,database,station_name,param,columns:str = Query(...)):
    ...
    if mode == 'date':
        ...
        file_name,info = draw.draw_specific_day_pro(database,station_name,column,param,sep='|',zone='Asia/Shanghai')
        image_token = str(uuid.uuid4())
        one_time_image_tokens[image_token] = (time.time(), file_name)
        return {"url":f"http://127.0.0.1/image?image_token={image_token}", "info": info}
    elif mode == 'hour':
        param = int(param) if not isinstance(param, int) else param
        
        file_name,info = draw.draw_last_hour_pro(database,station_name,column,param,sep='|',zone='Asia/Shanghai')
        image_token = str(uuid.uuid4())
        one_time_image_tokens[image_token] = (time.time(), file_name)
        return {"url":f"http://127.0.0.1/image?image_token={image_token}", "info": info}
    else:
