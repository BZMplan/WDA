...
async def v1_upload(item: cfg.meteorological_elements):
    ...
    # 计算衍生值
    can_calc_sea_pressure = (
        item.temperature is not None
        and item.pressure is not None
        and item.relative_humidity is not None
    )
    sea_level_pressure = (
        utils.calc_sea_level_pressure(
            item.temperature, item.pressure, item.relative_humidity, 27.5
        )
        if can_calc_sea_pressure
        else None
    )

    can_calc_dew_point = (
        item.temperature is not None and item.relative_humidity is not None
    )
    dew_point = (
        utils.calc_dew_point(item.temperature, item.relative_humidity)
        if can_calc_dew_point
        else None
    )
...
async def sensorlog(item: cfg.location) -> Dict[str, Any]:
    ...
    device_timestamp = int(item.locationTimestamp_since1970 or server_timestamp)
    day = time.strftime("%Y_%m_%d", time.localtime(device_timestamp))
    ...
    try:
        if not table_exists(table_name):
            create_sensorlog_table(table_name)
        insert_data(table_name, row)
    except Exception as e:
        return {
            "status": status.HTTP_500_INTERNAL_SERVER_ERROR,
